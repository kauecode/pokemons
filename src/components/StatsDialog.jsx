...
import { Dialog, Flex, Button, Text, Badge, Link } from "@radix-ui/themes";
...
const StatsDialog = ( {capturedList, setCapturedList} ) => {
  ...
  return (
    <Dialog.Root>
      <Dialog.Trigger>
        <Button size='4' >Your have {pokemonsTotal > 0 
        ? pokemonsTotal + " Pokemons"
        : "no Pokemons!" }</Button>
      </Dialog.Trigger>
      ...
      <Dialog.Content maxWidth="450px">
        ...
        <Dialog.Description size="3" mt="5">
          {pokemonsTotal === 0 
            ...
            : <Text>You have captured <Badge style={{ verticalAlign: "text-top" }} color="amber" variant="outline" radius="full">{pokemonsTotal}</Badge> Pokemons so far, here is your capture list:</Text>
          }          
        </Dialog.Description>
        ...
        <Flex direction="row" wrap="wrap" my='5' align='center' gap="3">
          ...
          <Button variant='soft' size='2' onClick={() => setCapturedList([])}>Release them all!</Button>
        </Flex>
        }
